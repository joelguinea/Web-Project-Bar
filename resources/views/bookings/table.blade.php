<div class="table-responsive">
    <table class="table" id="bookings-table">
        <thead>
            <tr>
                <th>Name</th>
        <th>Lastname</th>
        <th>Location</th>
        <th>Hotelname</th>
        <th>Date</th>
        <th>Roomtype</th>
        <th>Citizenship</th>
        <th>Gender</th>
                <th colspan="3">Action</th>
            </tr>
        </thead>
        <tbody>
        @foreach($bookings as $bookings)
            <tr>
                <td>{{ $bookings->Name }}</td>
            <td>{{ $bookings->Lastname }}</td>
            <td>{{ $bookings->Location }}</td>
            <td>{{ $bookings->Hotelname }}</td>
            <td>{{ $bookings->Date }}</td>
            <td>{{ $bookings->RoomType }}</td>
            <td>{{ $bookings->Citizenship }}</td>
            <td>{{ $bookings->Gender }}</td>
                <td width="120">
                    {!! Form::open(['route' => ['bookings.destroy', $bookings->id], 'method' => 'delete']) !!}
                    <div class='btn-group'>
                        <a href="{{ route('bookings.show', [$bookings->id]) }}" class='btn btn-default btn-xs'>
                            <i class="far fa-eye"></i>
                        </a>
                        <a href="{{ route('bookings.edit', [$bookings->id]) }}" class='btn btn-default btn-xs'>
                            <i class="far fa-edit"></i>
                        </a>
                        {!! Form::button('<i class="far fa-trash-alt"></i>', ['type' => 'submit', 'class' => 'btn btn-danger btn-xs', 'onclick' => "return confirm('Are you sure?')"]) !!}
                    </div>
                    {!! Form::close() !!}
                </td>
            </tr>
        @endforeach
        </tbody>
    </table>
</div>
